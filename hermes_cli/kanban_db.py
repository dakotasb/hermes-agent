...
def detect_crashed_workers(conn: sqlite3.Connection) -> list[str]:
    ...
    # Agents that write output files but exit before calling kanban_complete
    # (e.g. hitting max_iterations) are retried and typically succeed on the
    # second run because the workspace already has the output and the retry
    # detects it. The standard limit gives agents a fair retry window while
    # still surfacing persistent violators to a human.
    #
    # Systemic failures (3+ tasks with an identical error fingerprint) still
    # trip at failure_limit=1 to avoid hammering broken infra repeatedly.
